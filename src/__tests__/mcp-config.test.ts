...
describe('MCP Configuration Management', () => {
  ...
  describe('Environment Variable Setting', () => {
    it('should set OpenAI environment variables from config', () => {
      ...
      // Simulate the environment variable setting logic from getHandlers
    ...
    it('should set Google environment variables from config', () => {
      ...
      if (mockConfig.google?.apiKey) {
        process.env.GOOGLE_API_KEY = mockConfig.google.apiKey;
      }
      if (mockConfig.google?.baseURL) {
        process.env.GOOGLE_BASE_URL = mockConfig.google.baseURL;
      }
    ...
    it('should set Azure environment variables from config', () => {
      ...
      // Simulate the environment variable setting logic
      if (mockConfig.azure?.apiKey) {
        process.env.AZURE_API_KEY = mockConfig.azure.apiKey;
      }
      if (mockConfig.azure?.baseURL) {
        process.env.AZURE_BASE_URL = mockConfig.azure.baseURL;
      }
    ...
    it('should set XAI environment variables from config', () => {
      ...
      // Simulate the environment variable setting logic
      if (mockConfig.xai?.apiKey) {
        process.env.XAI_API_KEY = mockConfig.xai.apiKey;
      }
      if (mockConfig.xai?.baseURL) {
        process.env.XAI_BASE_URL = mockConfig.xai.baseURL;
      }
    ...
    it('should handle missing API keys gracefully', () => {
      ...
      // Should not set environment variables for missing keys
      if (mockConfig.openai?.apiKey) {
        process.env.OPENAI_API_KEY = mockConfig.openai.apiKey;
      }
      if (mockConfig.openai?.baseURL) {
    ...
    it('should handle missing base URLs gracefully', () => {
      ...
      // Should set API key but not base URL
      if (mockConfig.azure?.apiKey) {
        process.env.AZURE_API_KEY = mockConfig.azure.apiKey;
      }
      if (mockConfig.azure?.baseURL) {
  ...
  describe('Environment Variable Precedence', () => {
    ...
    it('should preserve existing environment variables when config is empty', () => {
      ...
      // Should not override existing env var when config doesn't have apiKey
      if (mockConfig.google?.apiKey) {
        process.env.GOOGLE_API_KEY = mockConfig.google.apiKey;
      }
      if (mockConfig.google?.baseURL) {
